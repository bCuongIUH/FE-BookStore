...
//   // Lấy danh sách đơn hàng của user từ API
...
//                             {order.items.reduce((sum, i) => sum + i.quantity, 0)} sản phẩm • {order.paymentMethod === "cod" ? "COD" : "Chuyển khoản"}
//                           </p>
//                         </div>
//                       </div>
...
//                           Xem chi tiết
//                         </Button>
//                         {order.status === "delivered" && <Button size="sm" variant="outline">Trả hàng</Button>}
...
export default function ProfilePage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      completed: { text: "Hoàn thành", color: "bg-green-600", icon: Award },
      refunded: { text: "Đã hoàn trả", color: "bg-blue-600", icon: RotateCcw },
      cancelled: { text: "Đã từ chối hoàn trả", color: "bg-red-500", icon: Clock },
      yeu_cau_hoan_tra: { text: "Yêu cầu hoàn trả", color: "bg-red-500", icon: Clock },
      paid: { text: "Hoàn trả", color: "bg-green-500",icon: Clock  },
    }
    const config = statusConfig[status as keyof typeof statusConfig] || statusConfig.pending
    const IconComponent = config.icon
    return (
